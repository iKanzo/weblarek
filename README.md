...
Документация : View Layer
Общие правила

Каждый компонент отвечает за свой участок DOM.

Компоненты не хранят данные.

Все действия пользователя → генерируют события.

Всё, что меняется на странице, происходит только:

по событию от моделей,

или по событию показа модального окна.

Вся разметка создаётся на основе:

шаблонов из HTML,

или структуры, описанной в TS.